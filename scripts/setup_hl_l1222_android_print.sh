...
# Ensure sbin paths are available when script is run with plain `bash script.sh`
...
status_report() {
  ...
  "$LPINFO_BIN" -m | grep -Ei 'brother|hl|brlaser|everywhere|generic|laser|mono|pcl' || true
  echo
  echo "=== CUPS status ==="
  "$LPSTAT_BIN" -t || true
}
...
install_dependencies() {
  ...
  if [[ "$ENABLE_BLUETOOTH" == "yes" ]]; then
    ...
    if ! sudo zypper --non-interactive install --no-recommends bluez; then
      err "Failed to install Bluetooth base package (bluez)."
      exit 1
    fi
    if ! sudo zypper --non-interactive install --no-recommends bluez-tools; then
      warn "Package bluez-tools not found in current repos; continuing without bt-adapter helper."
    fi
  fi
}
...
detect_usb_uri() {
  ...
  DEVICE_URI="$("$LPINFO_BIN" -v | awk 'BEGIN{IGNORECASE=1} /^direct[[:space:]]+usb:\/\// && /Brother|HL|L1222/ {print $2; exit}')"
...
select_model() {
  ...
  MODEL_NAME="$("$LPINFO_BIN" -m | awk 'BEGIN{IGNORECASE=1} /brlaser/ || (/Brother/ && /HL/ && /1200|1210|1212|1222|L12/) {print $1; exit}')"
  if [[ -n "$MODEL_NAME" ]]; then
  ...
  MODEL_NAME="$("$LPINFO_BIN" -m | awk 'BEGIN{IGNORECASE=1} /Generic/ && /laser|mono|pcl/ {print $1; exit}')"
  if [[ -n "$MODEL_NAME" ]]; then
...
create_or_replace_queue() {
  ...
  if "$LPSTAT_BIN" -p "$QUEUE_NAME" >/dev/null 2>&1; then
    confirm_high_risk "Queue '${QUEUE_NAME}' already exists and will be deleted/recreated."
    if ! sudo "$LPADMIN_BIN" -x "$QUEUE_NAME"; then
  ...
  if ! sudo "$LPADMIN_BIN" -p "$QUEUE_NAME" -E -v "$DEVICE_URI" -m "$MODEL_NAME"; then
...
install_hotfolder_service() {
  log "Installing hotfolder auto-print watcher (inotify-tools)..."
  ...
  if ! command -v inotifywait >/dev/null 2>&1; then
    if ! sudo zypper --non-interactive install --no-recommends inotify-tools; then
      err "Failed to install inotify-tools for hotfolder watcher."
      exit 1
    fi
  fi

  local watcher_sh
  watcher_sh="${TMP_DIR}/watch_print.sh"

  cat > "$watcher_sh" <<'SH'
#!/bin/bash
set -euo pipefail
QUEUE="${PRINT_QUEUE:-Brother_HL_L1222_V}"
HOTFOLDER="${PRINT_HOTFOLDER:-/var/spool/hll1222-hotfolder}"
mkdir -p "$HOTFOLDER"
inotifywait -m "$HOTFOLDER" -e close_write -e moved_to --format '%w%f' | while read -r file; do
  case "${file,,}" in
    *.pdf|*.jpg|*.jpeg|*.png|*.txt|*.tif|*.tiff)
...
SH
  ...
  if ! sudo install -m 0755 "$watcher_sh" "${SCRIPT_DIR}/watch_print.sh"; then
...
ExecStart=${SCRIPT_DIR}/watch_print.sh
Restart=always
RestartSec=3
...
install_upload_service_optional() {
  ...
  if ! python3 -c 'import multipart' >/dev/null 2>&1; then
    warn "python3-python-multipart is unavailable."
    warn "Skipping upload web service installation. Use IPP or hotfolder fallback."
    return 0
  fi
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "multipart/form-data" not in ctype:
            self.send_body(400, "Content-Type must be multipart/form-data")
            return
        ...
        if not name:
            self.send_body(400, "Missing filename")
            return
