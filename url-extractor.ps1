...
function Get-HtmlFragment {
    ...
    if ($startIndex -ge 0 -and $endIndex -gt $startIndex) {
        $startOffset = $RawHtml.IndexOf(':', $startIndex)
        $endOffset = $RawHtml.IndexOf(':', $endIndex)
        $startValue = $null
        $endValue = $null

        if ($startOffset -gt -1) {
            [void][int]::TryParse($RawHtml.Substring($startOffset + 1, 10).Trim(), [ref]$startValue)
        }

        if ($endOffset -gt -1) {
            [void][int]::TryParse($RawHtml.Substring($endOffset + 1, 10).Trim(), [ref]$endValue)
        }

        if ($startValue -ne $null -and $endValue -ne $null -and $endValue -gt $startValue -and $RawHtml.Length -ge $endValue) {
            return $RawHtml.Substring($startValue, $endValue - $startValue)
        }

        $fragmentStart = $startIndex + $startMarker.Length
        $fragmentEnd = $endIndex
        if ($fragmentEnd -gt $fragmentStart) {
            return $RawHtml.Substring($fragmentStart, $fragmentEnd - $fragmentStart)
        }
    }
...
function Write-Results {
    ...
    if (-not $Urls -or $Urls.Count -eq 0) {
        Write-Information 'No URLs found.' -InformationAction Continue
        return
    }
...
process {
    ...
    Write-Information ("Found {0} URL(s)." -f $collector.Count) -InformationAction Continue
}
